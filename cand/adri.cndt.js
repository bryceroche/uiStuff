		...
		ui:{
			...
			availability:{
				...
				get:function(onComplete){
					var svc = constants.urls.getAvailability + '?iref=' + constants.interview.id + '&uid=' + constants.interview.user;
					$.ajax({
						type: "GET",
						contentType: 'application/json',
						dataType: "json",
				...
				getSelectableTimes: function(date){
					var svc = constants.urls.getAvailabilityDate + '?iref=' + constants.interview.id + '&uid=' + constants.interview.user + '&adate=' + btoa(date);
					$.ajax({
						type: "GET",
						contentType: 'application/json',
						dataType: "json",
						url: svc,
						success: function(data){
							adri.ui.availability.drawSelectableTimes(data[0][0]);
						},
						error:function(xhr, ajaxOptions, error){
							console.log(xhr);
						}
			...
			loadToUI:function(data){
                ...
                var iCard = '<div id="interview-info-header-container" class="' + setClass('mainHeaderArea') + '">' +
                            ...
                            '</div > ' +
								...
								'</div>' +
								/** TO-DO **/
								/** Add Calendar Box Here -- box should have calendar with available days highlighted. **/
                            '<div id="calendar-' + interview['INTERVIEW_REFERENCE_ID'] + '"  class="' + setClass('calendarWrapper') + '">' +
									
								'</div>' +
								
							/** TO-DO **/
							/** Node area should display available interview start times once a date is selected from the calendar box **/
				...
				tMarkup = 	'<div class="' + setClass('interviewCardRow') + '">' + 
								'<div class="' + setClass('interviewCardField') + '">Start Time</div>' +
								'<div id="interview-info-start"></div>' +
							'</div>';
				$('#dtl-txt-' + interview['INTERVIEW_REFERENCE_ID']).append(tMarkup);
            },            
